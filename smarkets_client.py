...
from config import smarkets_configuration as configuration
...
class SmarketsClient:
    ...
    def get_related_markets(self,event_ids,market_name):
        markets = []
        markets_key = []
        i = 0
        ...
        for market in markets:
            markets_key.append([market['name'],market['id'],market['event_id']])
        markets_key=pd.DataFrame(markets_key)
        markets_key.columns=['Market_Name','Smarkets_market_id','Smarkets_event_id']
        return markets_key.loc[markets_key['Market_Name']==market_name][['Smarkets_market_id','Smarkets_event_id']]
    ...
    def get_related_contracts(self, market_ids):
        contracts = []
        i = 0
        chunk_size = configuration["api"]["chunk_size"]
    ...
    def get_related_volumes(self, market_ids):
        volumes = []
        i = 0
        chunk_size = configuration["api"]["chunk_size"]
        while i * chunk_size < len(market_ids):
            ...
            request_url = (
                f'''{configuration["api"]["base_url"]}markets/{markets_to_fetch}/volumes/'''
            )
            volumes += self._client_wrapper(request_url)['volumes']
            i += 1
        return volumes
    ...
    def get_quotes(self, market_ids: List[str]):
        ...
        while i * chunk_size < len(market_ids):
            ...
            i += 1
        quotes : list = quotes[0]
        
        if len(quotes)%len(market_ids) != 0:
            print('Error unexpcted number of selections relative to number of markets')
        else:
            step : int = int(len(quotes)/len(market_ids))
            quotes_list = [[i,j] for i,j in zip(quotes.keys(),quotes.values())]
            list_chunked = [quotes_list[i:i + step] for i in range(0, len(quotes_list), step)]
            
            #structure of this is still fucked just putting the same selection ids in under each market id, fuck me
                
            
            final_data : dict = {}
            for market_id,line in zip(market_ids,list_chunked):
                final_data[market_id] = {}
                for row in line:
                    selection_id = row[0]
                    quotes = row[1]
                    final_data[market_id][selection_id] = quotes
    ...
    def get_last_price(self, market_ids: List[str],contract_ids):
        quotes = []
        i = 0
        chunk_size = configuration["api"]["chunk_size"]
        while i * chunk_size < len(market_ids):
            ...
            request_url = f'{configuration["api"]["base_url"]}markets/{markets_to_fetch}/last_executed_prices/'
            quotes += [self._client_wrapper(request_url)]
            i += 1
        ...
        quotes=quotes_result['last_executed_prices'].values()
        output=[]
        for quote_element in quotes:
            for quote in quote_element:
                if quote['contract_id'] in contract_ids:
                    output.append([quote['contract_id'],quote['last_executed_price'],quote['timestamp']])
        return output
    ...
    def percent_to_decimal(self,percent):
        ...
        decimal=percent_to_decimal_dic[percent]
        #p=percent/10000
        
        return decimal#((1-p)/p)+1
